    ...
    RSpec.describe ParquetDocument do
      ...
      describe '.to_byte_string' do
        ...
        it 'writes a string that the reader can read' do
          ...
          reader = SimpleParquet::Reader::ParquetDocument.new(doc.to_byte_string)

          written_csv = reader.csv.to_s

          expect(written_csv).to eq(raw_csv)
        end
      end
    end
  end
